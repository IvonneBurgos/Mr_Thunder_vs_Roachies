...
level2.prototype = {
  ...
  preload: function() {
    ...
    this.game.load.audio('africa', ['assets/audio/end.mp3']);
    ...
    this.game.load.image('theend',"assets/images/GameImage-09.png");
    ...
    this.game.load.image('poison', 'assets/images/venom.png', 48, 58);
  ...
  create: function() {  
    ...
    for (var j=0; j< 15; j++){
      ...
      this.poison = this.add.group();
    for (var k=0; k < 3; k++){
        this.poison.create(this.game.rnd.between(60, 700), this.game.rnd.between(100, 700), 'poison', 1); 
    };
      //fisica
      this.game.physics.arcade.enable(this.roachies);
      this.game.physics.arcade.enable(this.groupCosas);
      this.game.physics.arcade.enable(this.poison);
    //texto
    this.groupCosas.setAll('body.immovable', true);
    this.roachies.setAll('body.immovable', true);
    this.poison.setAll('body.immovable', true);
  ...
  update: function() {
    ...
    this.game.physics.arcade.collide(this.spray, this.poison, this.collisionHandlerPoison, null, this);
  ...
  updateCounter:function() {
    ...
    collisionHandler:function(spray, roachies){
        ...
        if (this.spray.customParams.health > 0){
            ...
            this.refreshStats();
            if (this.spray.customParams.score == 150){
    ...
    collisionHandlerPoison:function(spray, poison){
